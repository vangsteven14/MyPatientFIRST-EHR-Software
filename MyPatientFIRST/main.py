...
#from view_patient_billing_ui import Ui_ViewPatientBilling                  # Still in progress
...
class MainWindow(QMainWindow):
        ...
        def populate_patient_ids(self):
            try:
                ...
                print("Selected Patient IDs and Names:\n", patient_ids)
                cursor.close()
                conn.close()
        ...
        def select_info_visits(self):
            ...
            # Convert the date string back to QDate
        ...
        def populate_patient_ids_billings(self):                            
            try:
                ...
                print("Selected Patient IDs, Visit IDs, and Reasons:\n", billings_patient_ids)
                cursor.close()
                conn.close()
        ...
        def update_info_billings(self):
            ...
            # Get the patient ID of the selected row and update the information if needed
            old_billing_id = self.result_table_billings.item(selected_row, 1).text().strip()
            ...
            if new_billing_info["billingID"]:
                ...
                if update_result:
                    QMessageBox.information(self, "Success", "Billing record updated successfully!",
                                            QMessageBox.StandardButton.Ok)
                else:
                    QMessageBox.information(self, "Warning", f"Fail to update the information: {update_result}, Please try again.",
                                            QMessageBox.StandardButton.Ok)
