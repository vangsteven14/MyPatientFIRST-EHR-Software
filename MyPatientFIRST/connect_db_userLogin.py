...
class LoginWindow(QWidget):
    def __init__(self):
        ...
        validator_username = QRegularExpressionValidator(tenChar_username, self.username_edit) # Allows only 10-digit numbers for Username
        self.username_edit.setValidator(validator_username)

        self.password_edit = QLineEdit()
        tenChar_password =  QRegularExpression(r'^[A-Za-z0-9!@#$%^&*()_+\-=\[\]{};\'":\\|,.<>/?]{1,10}$')
        validator_password = QRegularExpressionValidator(tenChar_password, self.password_edit) # Allows only 10 characters or less for Password
...
class CreateAccountWindow(QWidget):
    def __init__(self, db_connection=None):
        ...
        validator_new_username = QRegularExpressionValidator(tenChar_new_username, self.new_username_edit) # Allows only 10-digit numbers for Username
        ...
        validator_new_password = QRegularExpressionValidator(tenChar_new_password, self.new_password_edit) # Allows only 10 characters or less for Password
